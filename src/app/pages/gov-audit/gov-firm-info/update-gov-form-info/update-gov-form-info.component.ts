...
@Component({
  selector: 'app-update-gov-form-info',
  templateUrl: './update-gov-form-info.component.html',
  styleUrls: ['./update-gov-form-info.component.scss']
})
export class UpdateGovFormInfoComponent {
  formData: any = {};
  forms: any;
  citeies: any = [];
  provinces!: [];
  ...
  constructor(private callData: CallDataService,
              ...
              public dialogRef: MatDialogRef<UpdateGovFormInfoComponent>,
  ...
  ngGetForm(): void {
    this.callData.getGovFrimInfoForm().subscribe((res) => {
      this.forms = res.msg;
      if (this.data) {
  ...
  operation(action: any): void {
    ...
    this.callData.GovFrimInfoUpdate(this.formData).subscribe( (res) => {
      if (res.state) {
